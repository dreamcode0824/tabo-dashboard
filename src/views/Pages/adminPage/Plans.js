...
const PlanPage = () => {
  ...
  useEffect(() => {
    ...
    return new Promise((resolve, reject) => {
      ...
      if (errors.plan_changes.length > 0) {
        ...
        console.log(errors.plan_changes[len].plan_config, "error")
      } else {
  ...
  const couponCodeGenerate = (event) => {
    ...
    return new Promise((resolve, reject) => {
      ...
      console.log(business.filterBusinessList.id)
      dispatch(getPlanInformation(business.filterBusinessList.id))
    }).catch(errors => {
      if (errors) {
          ...
          <Col md="12" className="mx-auto">
            <Card className="card-user">
              <CardHeader className="px-4">
                <h5 className="title">{t("Choose your plan for")} {business.businessName ? business.businessName : business.businessLists[0].location_name}</h5>
              </CardHeader>
