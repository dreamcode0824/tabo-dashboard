...
function* login({ history, resolve, reject }) {
  ...
  try {
    ...
    if (result1.data) {
      console.log(result1.data.LoginClient.result)
      console.log(result1.data.LoginClient.token, result1.data.LoginClient.token[0])
      resolve("success")
      yield put({
      ...
      const result2 = yield call(existBusinessRequest, result1.data.LoginClient.result.id)
      if (result2.data) {
...
function* businessTypes({ history }) {
  try {
    ...
    if (result) {
      ...
      console.log(result.data.SaveBusinessTypes.businesses)
      history.push("/admin/business");
    }
  } catch (e) {
    console.log(e)
...
function* savePlanActions(value, couponId, id, price, resolve, reject) {
  console.log(value, couponId, id)
  try {
    const result = yield call(savePlanActionsRequest, value, couponId, id, price)
    if (result.message) {
  ...
  try {
    ...
    if (result.message) {
      ...
      if (result.data.plan_changes.length > 0) {
        ...
        console.log(result.data.plan_changes[0].plan_config.couponIds, "----------->")
